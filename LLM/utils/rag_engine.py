...
class SimpleRAG:
    ...
    def conditional_refresh(self) -> bool:
        """Reload knowledge base from disk if any KB file or updates log has changed. Returns True if a refresh was done."""
        if not self.kb_path.exists():
            return False
        last_ts = self.knowledge_base_timestamp.timestamp()
        for json_file in self.kb_path.glob("*.json"):
            try:
                if json_file.stat().st_mtime > last_ts:
                    self.load_knowledge_base()
                    self._load_updates_log()
                    return True
            except OSError:
