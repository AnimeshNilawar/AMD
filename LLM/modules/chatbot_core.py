...
from modules.m2_destination_suggester import DestinationSuggester, Destination
...
class WanderAIChatbotCoreV3:
    ...
    def _post_process_response(self, session: ChatSession, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Skip strict validation for task modules (already grounded) and for clarification fallbacks (template text)
        if result.get("path") == "TASK_MODULES" or result.get("module_used") == "modules":
            ...
            return result
        if result.get("path") == "FALLBACK" and result.get("type") == "clarification_needed":
            return result
    ...
    def _resolve_place_name_to_destination(self, place_name: str) -> Optional[Destination]:
        """Resolve a place name string to a Destination (from KB if found, else minimal)."""
        if not place_name or not str(place_name).strip():
            return None
        name = str(place_name).strip()
        # Try to find in RAG documents (e.g. places.json) for full details
        for doc in (self.rag.documents if self.rag else []):
            doc_name = doc.get("name") or doc.get("title") or ""
            if doc_name and name.lower() == doc_name.lower():
                return Destination(
                    name=doc_name,
                    category=doc.get("category", "General"),
                    match_score=90,
                    reasoning="From your request",
                    estimated_cost=doc.get("cost", "N/A"),
                    distance=doc.get("distance", "N/A"),
                    highlights=doc.get("tags", [])[:5] or [doc.get("description", "")[:80]],
                )
        # Not in KB: return a minimal Destination so itinerary can still be built
        return Destination(name=name.title(), category="General", match_score=80, reasoning="Mentioned in your request")
    ...
    def _extract_destination_from_context(
        ...
        """Extract destination from conversation context or intent. Returns a Destination or None."""
        # Check if user mentioned destination in intent (string → resolve to Destination)
        place_name = None
        if hasattr(intent, "destination") and intent.destination:
            place_name = str(intent.destination).strip()
        if not place_name and getattr(intent, "confirmation_place", None):
            place_name = str(intent.confirmation_place).strip()
        if place_name:
            return self._resolve_place_name_to_destination(place_name)

        # Check suggested places from session (may already be Destination objects)
        if session.suggested_places:
            first = session.suggested_places[0]
            if isinstance(first, Destination):
                return first
            return self._resolve_place_name_to_destination(str(first))
