...
├── server.py                # FastAPI HTTP server (POST /v1/chat, GET /v1/health, POST /v1/webhook)
...
A FastAPI server is provided so your backend can talk to WanderAI over HTTP:
...
|----------|--------|-------------|
| `/v1/chat` | POST | Send `{"message": "user text", "session_id": "optional-uuid"}`; returns response, data, session_id, module_used, etc. |
| `/v1/health` | GET | Health check (no LLM call). |
| `/v1/webhook` | POST | Knowledge-base update webhooks (body: `action`, `type`, `data`; optional `X-Webhook-Signature` if `WEBHOOK_SECRET` is set). |

Interactive docs: **http://localhost:8000/docs**

## Backend Integration

Use `api_adapter.handle_message()` as the primary entrypoint for servers, or call the HTTP API above. The adapter returns a dict with `response` (pretty string) and `data` (raw structured JSON). See `DOCUMENTATION.md` and `BACKEND_INTEGRATION.md` for examples and recommended patterns.
